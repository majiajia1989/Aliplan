...
import com.mjj.aliplan.GameObjectFactory.GameObjectType;
...
public class HitReactionComponent extends GameComponent {
	...
	public void hitVictim(GameObject parent, GameObject victim, int hitType,
		...
		if (hitAccepted) {
			...
			if (mDealHitSound != null
					&& (hitType == mDealHitSoundHitType || mDealHitSoundHitType == CollisionParameters.HitType.INVALID)) {
			...
			if (mSpawnOnDealHitObjectType != GameObjectType.INVALID
				...
				final float x = mAlignDealHitObjectToVictimX ? victim
						.getPosition().x : parent.getPosition().x;
				final float y = mAlignDealHitObjectToVictimY ? victim
						.getPosition().y : parent.getPosition().y;

				GameObjectFactory factory = sSystemRegistry.gameObjectFactory;
				GameObjectManager manager = sSystemRegistry.gameObjectManager;

				if (factory != null) {
					GameObject object = factory.spawn(
							mSpawnOnDealHitObjectType, x, y,
							parent.facingDirection.x < 0.0f);
	...
	public boolean receivedHit(GameObject parent, GameObject attacker,
			int hitType) {
		...
		if (mGameEventHitType == hitType
				&& mGameEventHitType != CollisionParameters.HitType.INVALID) {
			if (mLastGameEventTime < 0.0f
					|| gameTime > mLastGameEventTime + EVENT_SEND_DELAY) {
				LevelSystem level = sSystemRegistry.levelSystem;
				level.sendGameEvent(mGameEventOnHit, mGameEventIndexData, true);
			} else {
				// special case. If we're waiting for a hit type to spawn an
				// event and
				// another event has just happened, eat this hit so we don't
				// miss
		...
		case CollisionParameters.HitType.HIT:
			...
			if (!mForceInvincibility && !mInvincible && parent.life > 0
					&& !sameTeam) {
				parent.life -= 1;
		...
		if (hitType != CollisionParameters.HitType.INVALID) {
			if (mTakeHitSound != null && hitType == mTakeHitSoundHitType) {
				SoundSystem sound = sSystemRegistry.soundSystem;
				if (sound != null) {
					sound.play(mTakeHitSound, false,
							SoundSystem.PRIORITY_NORMAL);
				}
			}
	...
	public void update(float timeDelta, BaseObject parent) {
		...
		// This means that the lastReceivedHitType will persist for two frames,
		// giving all systems
		// a chance to react.
	...
	public void setSpawnGameEventOnHit(int hitType, int gameFlowEventType,
			int indexData) {
	...
	public final void setSpawnOnDealHit(int hitType, GameObjectType objectType,
			boolean alignToVictimX, boolean alignToVicitmY) {
