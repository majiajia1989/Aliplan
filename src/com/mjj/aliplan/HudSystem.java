...
public class HudSystem extends BaseObject {
	...
	private static final float STOMP_BUTTON_WIDTH = FLY_BUTTON_WIDTH
			* STOMP_BUTTON_SCALE;
	...
	public void reset() {
		...
		mMovementSliderBaseLocation.set(MOVEMENT_SLIDER_BASE_X,
				MOVEMENT_SLIDER_BASE_Y);
		mMovementSliderButtonLocation.set(MOVEMENT_SLIDER_BUTTON_X,
				MOVEMENT_SLIDER_BUTTON_Y);
	...
	public void setButtonDrawables(DrawableBitmap disabled,
			DrawableBitmap enabled, DrawableBitmap depressed,
			DrawableBitmap stompEnabled, DrawableBitmap stompDepressed,
			DrawableBitmap sliderBase, DrawableBitmap sliderButton,
	...
	public void setMovementSliderMode(boolean sliderOn) {
		...
		if (sliderOn) {
			...
			mFlyButtonLocation.set(params.gameWidth - FLY_BUTTON_WIDTH
					- FLY_BUTTON_X, FLY_BUTTON_Y);
			mStompButtonLocation.set(params.gameWidth - STOMP_BUTTON_WIDTH
					- STOMP_BUTTON_X, STOMP_BUTTON_Y);
		} else {
			mFlyButtonLocation.set(FLY_BUTTON_X, FLY_BUTTON_Y);
			mStompButtonLocation.set(STOMP_BUTTON_X, STOMP_BUTTON_Y);
		}
	...
	public void setMovementSliderOffset(float offset) {
		mMovementSliderButtonLocation.set(MOVEMENT_SLIDER_BUTTON_X
				+ (offset * (MOVEMENT_SLIDER_WIDTH / 2.0f)),
				MOVEMENT_SLIDER_BUTTON_Y);
	}
	...
	public void update(float timeDelta, BaseObject parent) {
		...
		if (manager != null && manager.getPlayer() != null) {
			...
			if (mFuelDrawable != null && mFuelBackgroundDrawable != null
					&& render != null && pool != null && factory != null
					&& params != null) {
				...
				if (mFuelBackgroundDrawable.getWidth() == 0) {
					...
					Texture backgroundTex = mFuelBackgroundDrawable
							.getTexture();
					mFuelBackgroundDrawable.resize(backgroundTex.width,
				...
				location.set(FUEL_BAR_EDGE_PADDING, params.gameHeight - height
						- FUEL_BAR_EDGE_PADDING);
				render.scheduleForDraw(mFuelBackgroundDrawable, location,
				...
				if (barWidth >= 1) {
					...
					if (bitmap != null) {
						...
						render.scheduleForDraw(bitmap, location,
			...
			if (mFlyButtonDisabledDrawable != null
					&& mFlyButtonEnabledDrawable != null
					&& mFlyButtonDepressedDrawable != null) {
				...
				render.scheduleForDraw(bitmap, mFlyButtonLocation,
						SortConstants.HUD, false);
			}

			if (mStompButtonEnabledDrawable != null
					&& mStompButtonDepressedDrawable != null) {
				...
				render.scheduleForDraw(bitmap, mStompButtonLocation,
						SortConstants.HUD, false);
			}

			if (mMovementSliderMode && mMovementSliderBaseDrawable != null
					&& mMovementSliderButtonDrawable != null) {
				...
				if (mMovementSliderButtonDepressedDrawable.getWidth() == 0) {
					// first time init
					Texture tex = mMovementSliderButtonDepressedDrawable
							.getTexture();
					mMovementSliderButtonDepressedDrawable.resize(tex.width,
							tex.height);
				}
				...
				render.scheduleForDraw(mMovementSliderBaseDrawable,
						mMovementSliderBaseLocation, SortConstants.HUD, false);
				render.scheduleForDraw(bitmap, mMovementSliderButtonLocation,
						SortConstants.HUD + 1, false);
			...
			if (mCoinDrawable != null) {
				if (mCoinDrawable.getWidth() == 0) {
					...
					mCoinLocation.x = (params.gameWidth / 2.0f) - tex.width
							/ 2.0f;
					mCoinLocation.y = params.gameHeight - tex.height
							- COLLECTABLE_EDGE_PADDING;
				}

				render.scheduleForDraw(mCoinDrawable, mCoinLocation,
						SortConstants.HUD, false);
			...
			if (mRubyDrawable != null) {
				if (mRubyDrawable.getWidth() == 0) {
					...
					mRubyLocation.y = params.gameHeight - tex.height
							- COLLECTABLE_EDGE_PADDING;
				}
				render.scheduleForDraw(mRubyDrawable, mRubyLocation,
						SortConstants.HUD, false);
		...
		if (mShowFPS) {
			if (mFPSDigitsChanged) {
				...
				mFPSLocation
						.set(params.gameWidth
								- 10.0f
								- ((count + 1) * (mDigitDrawables[0].getWidth() / 2.0f)),
								10.0f);
		...
		if (mFading && factory != null) {
			...
			if (percentComplete < 1.0f || !mFadeIn) {
				...
				if (bitmap != null) {
					...
					bitmap.setCrop(0, mFadeTexture.height, mFadeTexture.width,
							mFadeTexture.height);
					bitmap.setOpacity(opacityValue);
					render.scheduleForDraw(bitmap, Vector2.ZERO,
							SortConstants.FADE, false);
				}
			}
			...
			if (percentComplete >= 1.0f
				...
				if (level != null) {
					level.sendGameEvent(mFadePendingEventType,
							mFadePendingEventIndex, false);
					mFadePendingEventType = GameFlowEvent.EVENT_INVALID;
					mFadePendingEventIndex = 0;
				}
			}
