	...
	public void update(float timeDelta, BaseObject parent) {
		if (mBlurTarget != null) {
			...
			if (mTimeSinceLastStep > mStepDelay) {
				DrawableBitmap drawable = (DrawableBitmap) mBlurTarget
						.getDrawable();
				if (drawable != null) {
					...
					mHistory[mCurrentStep].position.set(((GameObject) parent)
							.getPosition());
			...
			// draw each step
			for (int x = 0; x < STEP_COUNT; x++) {
				final int step = (startStep - x) < 0 ? (STEP_COUNT + (startStep - x))
						: (startStep - x);
				final BlurRecord record = mHistory[step];
				if (record.texture != null) {
					DrawableBitmap stepImage = sSystemRegistry.drawableFactory
					...
					stepImage.setCrop(record.crop[0], record.crop[1],
							record.crop[2], -record.crop[3]);
