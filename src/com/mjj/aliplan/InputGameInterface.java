...
public class InputGameInterface extends BaseObject {
	...
	// make the control more twitchy, while decreasing it will make the control
	// more precise.
	...
	public void update(float timeDelta, BaseObject parent) {
		...
		if (mUseOnScreenControls) {
			...
			if (sliderTouch != null) {
				...
				final float center = ButtonConstants.MOVEMENT_SLIDER_X
						+ halfWidth;
				final float offset = sliderTouch.getX() - center;
				float magnitudeRamp = Math.abs(offset) > halfWidth ? 1.0f
						: (Math.abs(offset) / halfWidth);

				final float magnitude = magnitudeRamp * Utils.sign(offset)
						* SLIDER_FILTER * mMovementSensitivity;
			...
			if (trackball.getLastPressedTime() > Math.max(leftPressedTime,
					rightPressedTime)) {
				// The trackball never goes "up", so force it to turn off if it
				// wasn't triggered in the last frame.
				// What follows is a bunch of code to filter trackball events
				// into something like a dpad event.
				// The goals here are:
				// - For roll events that occur in quick succession to
				// accumulate.
				// - For roll events that occur with more time between them,
				// lessen the impact of older events
				...
				if (gameTime - trackball.getLastPressedTime() < ROLL_TIMEOUT) {
					...
					if (gameTime - mLastRollTime <= delay) {
						newX = mDirectionalPad.getX()
								+ (trackball.getX() * ROLL_FILTER * mMovementSensitivity);
						newY = mDirectionalPad.getY()
								+ (trackball.getY() * ROLL_FILTER * mMovementSensitivity);
					} else {
						float oldX = mDirectionalPad.getX() != 0.0f ? mDirectionalPad
								.getX() / 2.0f : 0.0f;
						float oldY = mDirectionalPad.getX() != 0.0f ? mDirectionalPad
								.getX() / 2.0f : 0.0f;
						newX = oldX
								+ (trackball.getX() * ROLL_FILTER * mMovementSensitivity);
						newY = oldY
				...
				if (leftPressedTime > rightPressedTime) {
					xMagnitude = -left.getMagnitude() * KEY_FILTER
							* mMovementSensitivity;
					pressTime = leftPressedTime;
				} else {
					xMagnitude = right.getMagnitude() * KEY_FILTER
							* mMovementSensitivity;
		...
		// when on-screen movement controls are on, the fly and attack buttons
		// are flipped.
		...
		final InputButton clickButton = keys[KeyEvent.KEYCODE_DPAD_CENTER]; // special
																			// case

		final InputXY stompTouch = touch.findPointerInRegion(
				stompButtonRegionX, ButtonConstants.STOMP_BUTTON_REGION_Y,
				ButtonConstants.STOMP_BUTTON_REGION_WIDTH,
				ButtonConstants.STOMP_BUTTON_REGION_HEIGHT);

		if (mUseClickButtonForAttack && clickButton.getPressed()) {
			mAttackButton.press(clickButton.getLastPressedTime(),
					clickButton.getMagnitude());
		} else if (attackKey.getPressed()) {
			mAttackButton.press(attackKey.getLastPressedTime(),
					attackKey.getMagnitude());
		} else if (stompTouch != null) {
			// Since touch events come in constantly, we only want to press the
			// attack button
			// here if it's not already down. That makes it act like the other
			// buttons (down once then up).
		...
		// This doesn't seem like exactly the right place to write to the HUD,
		// but on the other hand,
		// putting this code elsewhere causes dependencies between exact HUD
		// content and physics, which
		// we sometimes wish to avoid.
		final HudSystem hud = sSystemRegistry.hudSystem;
		if (hud != null) {
			hud.setButtonState(mJumpButton.getPressed(),
					mAttackButton.getPressed(), mDirectionalPad.getPressed());
	...
	private float filterOrientationForMovement(float magnitude) {
		...
		return deadZoneFilter(scaledMagnitude, mOrientationDeadZoneMin,
				mOrientationDeadZoneMax, mOrientationDeadZoneScale);
	}

	private float deadZoneFilter(float magnitude, float min, float max,
			float scale) {
