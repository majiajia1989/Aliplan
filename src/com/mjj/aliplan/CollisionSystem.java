...
public class CollisionSystem extends BaseObject {
	...
	private byte[] mWorkspaceBytes; // Included here to avoid runtime allocation
									// during file io.
	...
	public CollisionSystem() {
		...
		mTemporarySegments = new FixedSizeArray<LineSegment>(
				MAX_TEMPORARY_SEGMENTS);
		mPendingTemporarySegments = new FixedSizeArray<LineSegment>(
				MAX_TEMPORARY_SEGMENTS);
	...
	/**
	 * Casts a ray into the collision world. The ray is bound by the start and
	 * end points supplied. The first intersecting segment that is found is
	 * returned; in the case where more than one segment is found, the segment
	 * closest to the start point is returned.
	 * 
	 * @param startPoint
	 ...
	 *            If set, only segments with normals that oppose this direction
	 *            will be counted as valid intersections. If null, all
	 *            intersecting segments will be considered valid.
	 * @param hitPoint
	 *            The point of intersection between a ray and a surface, if one
	 *            is found.
	 * @param hitNormal
	 *            The normal of the intersecting surface if an intersection is
	 *            found.
	 * @param excludeObject
	...
	public boolean castRay(Vector2 startPoint, Vector2 endPoint,
			Vector2 movementDirection, Vector2 hitPoint, Vector2 hitNormal,
		...
		if (mCollisionTiles != null
				&& executeRay(startPoint, endPoint, hitPoint, hitNormal,
		...
		if (mTemporarySegments.getCount() > 0) {
			...
			if (testSegmentAgainstList(mTemporarySegments, startPoint,
					endPoint, tempHitPoint, tempHitNormal, movementDirection,
					excludeObject)) {
				if (hit) {
					// Check to see whether this collision is closer to the one
					// we already found or
					// not.
					final float firstCollisionDistance = startPoint
							.distance2(hitPoint);
					if (firstCollisionDistance > startPoint
							.distance2(tempHitPoint)) {
	...
	public boolean testBox(float left, float right, float top, float bottom,
		...
		if (!testDynamicSurfacesOnly) {
			...
			final int startTileX = Utils.clamp((int) (startX / mTileWidth), 0,
					mWorld.getWidth() - 1);
			final int endTileX = Utils.clamp((int) (endX / mTileWidth), 0,
					mWorld.getWidth() - 1);
			final int startTileY = Utils.clamp((int) (startY / mTileHeight), 0,
					mWorld.getHeight() - 1);
			final int endTileY = Utils.clamp((int) (endY / mTileHeight), 0,
					mWorld.getHeight() - 1);
			...
			for (int y = startTileY; y != endTileY + yIncrement; y += yIncrement) {
				for (int x = startTileX; x != endTileX + xIncrement; x += xIncrement) {
					...
					if (tileIndex >= 0 && tileIndex < mCollisionTiles.length
						...
						boolean hit = testBoxAgainstList(
								mCollisionTiles[tileIndex].segments,
								tileSpaceLeft, tileSpaceRight, tileSpaceTop,
								tileSpaceBottom, movementDirection,
								excludeObject, worldTileOffset, hitPoints);
		...
		// temporary segments
		boolean tempHit = testBoxAgainstList(mTemporarySegments, left, right,
				top, bottom, movementDirection, excludeObject, Vector2.ZERO,
				hitPoints);
	...
	public void update(float timeDelta, BaseObject parent) {
		...
		// Temporary surfaces must persist for one frame in order to be reliable
		// independent of
		// frame execution order. So each frame we queue up inserted segments
		// and then swap them
	...
	/**
	 * Shoots a ray through the collision world. This function is similar to
	 * executeRay() below, except that it is optimized for straight lines
	 * (either completely horizontal or completely vertical).
	 * 
	 * @param startPoint
	 ...
	 * @param hitPoint
	 *            Set to the intersection coordinates if an intersection is
	 *            found.
	 * @param hitNormal
	 *            Set to the normal of the intersecting surface if an
	 *            intersection is found.
	 * @param visitor
	 *            Class defining what work to perform at each tile step.
	 * @return The index of the tile that intersected the ray, or -1 if no
	 *         intersection was found.
	 */
	protected int executeStraigtRay(final Vector2 startPoint,
			final Vector2 endPoint, final int startTileX, final int startTileY,
			final int endTileX, final int endTileY, final int deltaX,
			final int deltaY, Vector2 hitPoint, Vector2 hitNormal,
			TileVisitor visitor) {
		...
		for (int x = 0; x < distance; x++) {
			...
			if (tileIndex >= 0 && tileIndex < mCollisionTiles.length
					&& mCollisionTiles[tileIndex] != null) {
				if (visitor.visit(mCollisionTiles[tileIndex], startPoint,
						endPoint, hitPoint, hitNormal, currentX, currentY)) {
					hitTile = tileIndex;
					break;
				}
			}
	...
	/**
	 * Shoots a ray through the collision world. Since the collision world is a
	 * 2D array of tiles, this algorithm traces a line in tile space and tests
	 * against each non-empty tile it visits. The Bresenham line algorithm is
	 * used for the actual traversal, but the action taken at each tile is
	 * defined by the visitor class passed to this function.
	 * 
	 * @param startPoint
	 *            The starting point for the ray, in world space.
	 * @param endPoint
	...
	protected int executeRay(Vector2 startPoint, Vector2 endPoint,
		...
		if (deltaX == 0 || deltaY == 0) {
			hitTile = executeStraigtRay(startPoint, endPoint, startTileX,
					startTileY, endTileX, endTileY, deltaX, deltaY, hitPoint,
					hitNormal, visitor);
		} else {
			...
			// Note: I'm deviating from the Bresenham algorithm here by adding
			// one to force the end
			// tile to be visited.
			final int lateralDelta = (endTileX > 0 && endTileX < worldWidth - 1) ? Math
					.abs(deltaX) + 1 : Math.abs(deltaX);
			final int verticalDelta = (endTileY > 0 && endTileY < worldHeight - 1) ? Math
					.abs(deltaY) + 1 : Math.abs(deltaY);
			...
			if (lateralDelta >= verticalDelta) {
				...
				for (int i = 0; i < lateralDelta; i++) {
					final int tileIndex = tileArray[currentX][worldHeightMinusOne
							- currentY];
					if (tileIndex >= 0 && tileIndex < mCollisionTiles.length
							&& mCollisionTiles[tileIndex] != null) {
						if (visitor.visit(mCollisionTiles[tileIndex],
	...
	protected static boolean testSegmentAgainstList(
		...
		for (int x = 0; x < count; x++) {
			...
			// If a movement direction has been passed, filter out invalid
			// surfaces by ignoring
			// those that do not oppose movement. If no direction has been
			// passed, accept all
			// surfaces.
			final float dot = movementDirection.length2() > 0.0f ? movementDirection
					.dot(segment.mNormal) : -1.0f;
			...
			if (dot < 0.0f
				...
				if (!foundHit || closestDistance > distance) {
					...
					// Store the components on their own so we don't have to
					// allocate a vector
					// in this loop.
					hitX = hitPoint.x;
					hitY = hitPoint.y;
	...
	protected static boolean testBoxAgainstList(
			...
			float top, float bottom, Vector2 movementDirection,
			GameObject excludeObject, Vector2 outputOffset,
	...
	public boolean loadCollisionTiles(InputStream stream) {
		...
		// TODO: this is a hack. I really should only allocate an array that is
		// the size of the
		// tileset, but at this point I don't actually know that size, so I
		// allocate a buffer that's
			...
			if (signature == 52) {
				...
				if (byteStream.available() >= size) {
					for (int x = 0; x < tileCount; x++) {
						...
						if (mCollisionTiles[tileIndex] == null
								&& segmentCount > 0) {
							mCollisionTiles[tileIndex] = new CollisionTile(
									segmentCount);
						}

						for (int y = 0; y < segmentCount; y++) {
							byteStream.read(mWorkspaceBytes, 0, 4);
							final float startX = Utils
									.byteArrayToFloat(mWorkspaceBytes);
							byteStream.read(mWorkspaceBytes, 0, 4);
							final float startY = Utils
									.byteArrayToFloat(mWorkspaceBytes);
							byteStream.read(mWorkspaceBytes, 0, 4);
							final float endX = Utils
									.byteArrayToFloat(mWorkspaceBytes);
							byteStream.read(mWorkspaceBytes, 0, 4);
							final float endY = Utils
									.byteArrayToFloat(mWorkspaceBytes);
							byteStream.read(mWorkspaceBytes, 0, 4);
							final float normalX = Utils
									.byteArrayToFloat(mWorkspaceBytes);
							byteStream.read(mWorkspaceBytes, 0, 4);
							final float normalY = Utils
									.byteArrayToFloat(mWorkspaceBytes);

							// TODO: it might be wise to pool line segments. I
							// don't think that
							// this data will be loaded very often though, so
							// this is ok for now.
	...
	/**
	 * An interface for visiting tiles during a ray cast. Implementations of
	 * TileVisitor can be passed to executeRay(); the visit() function will be
	 * invoked for each tile touched by the ray until the traversal is completed
	...
	public abstract class TileVisitor extends AllocationGuard {
		...
		public abstract boolean visit(CollisionTile tile, Vector2 startPoint,
				Vector2 endPoint, Vector2 hitPoint, Vector2 hitNormal,
				int tileX, int tileY);
	}

	/**
	 * TileTestVisitor tests the ray against a list of segments assigned to each
	 * tile. If any segment in any tile of the ray's path is found to be
	 * intersecting with the ray, traversal stops and intersection information
	 * is recorded.
	 */
	protected class TileTestVisitor extends TileVisitor {
		// These vectors are all temporary storage variables allocated as class
		// members to avoid
		// runtime allocation.
		...
		/**
		 * Sets the visitor up for a ray test. Initializes the size of the tiles
		 * and the direction of movement by which intersections should be
		 * filtered.
		 */
		public void setup(Vector2 movementDirection, int tileWidth,
				int tileHeight) {
			if (movementDirection != null) {
		 ...
		 */
		@Override
		public boolean visit(CollisionTile tile, Vector2 startPoint,
				Vector2 endPoint, Vector2 hitPoint, Vector2 hitNormal,
				int tileX, int tileY) {
			...
			// find all the hits in the tile and pick the closest to the start
			// point.
			boolean foundHit = testSegmentAgainstList(tile.segments,
					mTileSpaceStart, mTileSpaceEnd, hitPoint, hitNormal,
					mDelta, null);

			if (foundHit) {
				// The hitPoint is in tile space, so convert it back to world
				// space.
	...
	/**
	 * A class describing a single surface in the collision world. Surfaces are
	 * stored as a line segment and a normal. The normal must be normalized (its
	 * length must be 1.0) and should describe the direction that the segment
	 * "pushes against" in a collision.
	 */
	protected class LineSegment extends AllocationGuard {
		...
		/**
		 * Checks to see if these lines intersect by projecting one onto the
		 * other and then assuring that the collision point is within the range
		 * of each segment.
		 */
		public boolean calculateIntersection(Vector2 otherStart,
				Vector2 otherEnd, Vector2 hitPoint) {
		...
		public boolean calculateIntersectionBox(float left, float right,
				float top, float bottom, Vector2 hitPoint) {
