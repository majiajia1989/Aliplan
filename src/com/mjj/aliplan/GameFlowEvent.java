...
public class GameFlowEvent implements Runnable {
	...
	public void post(int event, int index, Context context) {
		if (context instanceof AndouKun) {
			DebugLog.d("GameFlowEvent", "Post Game Flow Event: " + event + ", "
					+ index);
			mEventCode = event;
			mDataIndex = index;
	...
	public void postImmediate(int event, int index, Context context) {
		if (context instanceof AndouKun) {
			DebugLog.d("GameFlowEvent", "Execute Immediate Game Flow Event: "
					+ event + ", " + index);
			mEventCode = event;
			mDataIndex = index;
			mMainActivity = (AndouKun) context;
	...
	public void run() {
		if (mMainActivity != null) {
			DebugLog.d("GameFlowEvent", "Execute Game Flow Event: "
					+ mEventCode + ", " + mDataIndex);
			mMainActivity.onGameFlowEvent(mEventCode, mDataIndex);
			mMainActivity = null;
		}
	}
