...
public class GameObjectCollisionSystem extends BaseObject {
	...
	public GameObjectCollisionSystem() {
		super();
		mObjects = new FixedSizeArray<CollisionVolumeRecord>(
				MAX_COLLIDING_OBJECTS);
	...
	/**
	 * Adds a game object, and its related volumes, to the dynamic collision
	 * world for one frame. Once registered for collisions the object may damage
	 * other objects via attack volumes or receive damage from other volumes via
	 * vulnerability volumes.
	 * 
	 * @param object
	 *            The object to consider for collision.
	 * @param reactionComponent
	 *            A HitReactionComponent to notify when an intersection is
	 *            calculated. If null, the intersection will still occur and no
	 *            notification will be sent.
	 * @param boundingVolume
	 *            A volume that describes the game object in space. It should
	 *            encompass all of the attack and vulnerability volumes.
	 * @param attackVolumes
	 *            A list of volumes that can hit other game objects. May be
	 *            null.
	 * @param vulnerabilityVolumes
	 *            A list of volumes that can receive hits from other game
	 *            objects. May be null.
	 */
	...
	public void update(float timeDelta, BaseObject parent) {
		...
		for (int x = 0; x < count; x++) {
			...
			final float maxX = record.boundingVolume.getMaxXPosition(sFlip)
					+ position.x;
			for (int y = x + 1; y < count; y++) {
				...
				if (otherPosition.x
						+ other.boundingVolume.getMinXPosition(sOtherFlip) > maxX) {
					// These objects can't possibly be colliding. And since the
					// list is sorted,
					// there are no potentially colliding objects after this
					// object
					...
					if (testRequired
							&& record.boundingVolume.intersects(position,
									sFlip, other.boundingVolume, otherPosition,
									sOtherFlip)) {
						...
						final int hit = testAttackAgainstVulnerability(
								...
								other.vulnerabilityVolumes, position,
								otherPosition, sFlip, sOtherFlip);
						if (hit != HitType.INVALID) {
							boolean hitAccepted = false;
							if (other.reactionComponent != null) {
								hitAccepted = other.reactionComponent
										.receivedHit(other.object,
												record.object, hit);
							}
							if (record.reactionComponent != null) {
								record.reactionComponent.hitVictim(
										record.object, other.object, hit,
										hitAccepted);
							}
						...
						final int hit2 = testAttackAgainstVulnerability(
								other.attackVolumes,
								record.vulnerabilityVolumes, otherPosition,
								position, sOtherFlip, sFlip);
						if (hit2 != HitType.INVALID) {
							boolean hitAccepted = false;
							if (record.reactionComponent != null) {
								hitAccepted = record.reactionComponent
										.receivedHit(record.object,
												other.object, hit2);
							}
							if (other.reactionComponent != null) {
								other.reactionComponent.hitVictim(other.object,
										record.object, hit2, hitAccepted);
							}
			...
			// This is a little tricky. Since we always sweep forward in the
			// list it's safe
			// to invalidate the current record after we've tested it. This way
			// we don't have to
	...
	/**
	 * Compares the passed list of attack volumes against the passed list of
	 * vulnerability volumes and returns a hit type if an intersection is found.
	 * 
	 * @param attackVolumes
	 ...
	 * @return The hit type of the first attacking volume that intersects a
	 *         vulnerability volume, or HitType.INVALID if no intersections are
	 *         found.
	 */
	private int testAttackAgainstVulnerability(
			FixedSizeArray<CollisionVolume> attackVolumes,
			FixedSizeArray<CollisionVolume> vulnerabilityVolumes,
			Vector2 attackPosition, Vector2 vulnerabilityPosition,
		...
		if (attackVolumes != null && vulnerabilityVolumes != null) {
			...
			for (int x = 0; x < attackCount
					&& intersectionType == HitType.INVALID; x++) {
				...
				if (hitType != HitType.INVALID) {
					final int vulnerabilityCount = vulnerabilityVolumes
							.getCount();
					for (int y = 0; y < vulnerabilityCount; y++) {
						final CollisionVolume vulnerabilityVolume = vulnerabilityVolumes
								.get(y);
						final int vulnerableType = vulnerabilityVolume
								.getHitType();
						if (vulnerableType == HitType.INVALID
								|| vulnerableType == hitType) {
							if (attackVolume.intersects(attackPosition,
									attackFlip, vulnerabilityVolume,
									vulnerabilityPosition, vulnerabilityFlip)) {
	...
	private final void drawDebugVolumes(CollisionVolumeRecord record) {
		...
		if (mDrawDebugBoundingVolume) {
			...
			sSystemRegistry.debugSystem.drawShape(
					position.x + boundingVolume.getMinXPosition(sFlip),
					position.y + boundingVolume.getMinYPosition(sFlip),
					boundingVolume.getMaxX() - boundingVolume.getMinX(),
					boundingVolume.getMaxY() - boundingVolume.getMinY(),
					DebugSystem.SHAPE_CIRCLE, DebugSystem.COLOR_OUTLINE);
		}
		if (mDrawDebugCollisionVolumes) {
				...
				for (int y = 0; y < attackVolumeCount; y++) {
					...
					sSystemRegistry.debugSystem
							.drawShape(
									...
									volume.getClass() == AABoxCollisionVolume.class ? DebugSystem.SHAPE_BOX
			...
			if (record.vulnerabilityVolumes != null) {
				final int vulnVolumeCount = record.vulnerabilityVolumes
						.getCount();
	...
	public void setDebugPrefs(boolean drawBoundingVolumes,
	...
	private class CollisionVolumeRecordPool extends
			TObjectPool<CollisionVolumeRecord> {
	...
	/**
	 * Comparator for game objects that considers the world position of the
	 * object's bounding volume and sorts objects from left to right on the x
	 * axis.
	 */
	public final static class CollisionVolumeComparator implements
			Comparator<CollisionVolumeRecord> {
		private static CollisionVolume.FlipInfo sCompareFlip = new CollisionVolume.FlipInfo();

		public int compare(CollisionVolumeRecord object1,
