		...
		if (mSprite != null) {
			...
			if (timeSinceTriggered > openAnimationLength) {
				...
				if (mState == STATE_CLOSING) {
					...
					// if we're in the middle of one and need to go to the
					// other,
					// we can start the new one mid-way through so that the door
					// appears to
					// simply reverse direction.
					timeOffset = openAnimationLength
		...
		if (mSprite != null) {
			...
			if (timeSinceTriggered > mStayOpenTime + closeAnimationLength) {
				...
				if (mState == STATE_OPENING) {
					timeOffset = closeAnimationLength
							- mSprite.getCurrentAnimationTime();
				}
	...
	public void update(float timeDelta, BaseObject parent) {
		if (mChannel != null) {
			if (mChannel.value != null
					&& mChannel.value instanceof ChannelSystem.ChannelFloatValue) {
		...
		if (mSprite != null) {
			...
			if (mSprite.getCurrentAnimation() == Animation.OPENING
					&& mState == STATE_CLOSED) {
				mSprite.playAnimation(Animation.CLOSING);
				mState = STATE_CLOSING;
			}
