...
public class LevelBuilder extends BaseObject {
	...
	public GameObject buildBackground(int backgroundImage, int levelWidth,
			int levelHeight) {
		...
		if (textureLibrary != null) {
			...
			if (backgroundResource > -1) {
				...
				// The background image is ideally 1.5 times the size of the
				// largest screen axis
				// (normally the width, but just in case, let's calculate it).
				final int idealSize = (int) Math.max(params.gameWidth * 1.5f,
						params.gameHeight * 1.5f);
				int width = idealSize;
				int height = idealSize;

				ScrollerComponent scroller3 = new ScrollerComponent(0.0f, 0.0f,
						width, height,
						textureLibrary.allocateTexture(backgroundResource));
				scroller3.setRenderComponent(backgroundRender);

				// Scroll speeds such that the background will evenly match the
				// beginning
				// and end of the level. Don't allow speeds > 1.0, though; that
				// would be faster than
				// the foreground, which is disorienting and looks like
				// rotation.
				final float scrollSpeedX = Math.min(
						(float) (width - params.gameWidth)
								/ (levelWidth - params.gameWidth), 1.0f);
				final float scrollSpeedY = Math.min(
						(float) (height - params.gameHeight)
								/ (levelHeight - params.gameHeight), 1.0f);
	...
	public void addTileMapLayer(GameObject background, int priority,
			float scrollSpeed, int width, int height, int tileWidth,
			int tileHeight, TiledWorld world, int theme) {
		...
		TiledVertexGrid bg = new TiledVertexGrid(
				textureLibrary.allocateTexture(tileMapIndex), width, height,
				tileWidth, tileHeight);
		bg.setWorld(world);

		// TODO: The map format should really just output independent speeds for
		// x and y,
		// but as a short term solution we can assume parallax layers lock in
		// the smaller
