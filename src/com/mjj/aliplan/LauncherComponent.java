...
public class LauncherComponent extends GameComponent {
	...
	public void update(float timeDelta, BaseObject parent) {
		...
		if (mShot != null) {
			if (mShot.life <= 0) {
				...
				// TODO: this is unreliable. We should have a "notify on death"
				// event or something.
				mShot = null;
			} else {
	...
	public void prepareToLaunch(GameObject object, GameObject parentObject) {
		if (mShot != object) {
			if (mShot != null) {
				// We already have a shot loaded and we are asked to shoot
				// something else.
	...
	private void fire(GameObject object, GameObject parentObject, float mAngle) {
		...
		mLaunchDirection
				.set((float) Math.sin(mAngle), (float) Math.cos(mAngle));
		...
		if (mLaunchEffect != GameObjectFactory.GameObjectType.INVALID) {
			...
			if (factory != null && manager != null) {
				...
				GameObject effect = factory
						.spawn(mLaunchEffect,
								position.x
										+ (mLaunchEffectOffsetX * parentObject.facingDirection.x),
								position.y
										+ (mLaunchEffectOffsetY * parentObject.facingDirection.y),
								false);
				if (effect != null) {
					manager.add(effect);
				}
	...
	public void setup(float angle, float magnitude, float launchDelay,
			float postLaunchDelay, boolean driveActions) {
		mAngle = angle;
	...
	public void setLaunchEffect(GameObjectFactory.GameObjectType effectType,
