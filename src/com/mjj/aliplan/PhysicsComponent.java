...
public class PhysicsComponent extends GameComponent {
	...
	public void update(float timeDelta, BaseObject parent) {
		...
		if (surfaceNormal.length2() > 0.0f) {
			resolveCollision(currentVelocity, impulseVector, surfaceNormal,
					impulseVector);
		}

		VectorPool vectorPool = sSystemRegistry.vectorPool;

		// if our speed is below inertia, we need to overcome inertia before we
		// can move.
		...
		GravityComponent gravity = parentObject
		...
		if (touchingFloor && currentVelocity.y <= 0.0f
			...
			// static.
			float frictionCoeffecient = Math.abs(currentVelocity.x) > 0.0f ? getDynamicFrictionCoeffecient()
					: getStaticFrictionCoeffecient();
			...
			if (maxFriction > Math.abs(newVelocity.x)) {
				...
				newVelocity.x = (newVelocity.x - (maxFriction * Utils
						.sign(newVelocity.x)));
			}
		}
		...
		// physics-based movements means constant acceleration, always. set the
		// target to the
		// velocity.
	...
	protected void resolveCollision(Vector2 velocity, Vector2 impulse,
		...
		final float dotRelativeAndNormal = relativeVelocity
				.dot(collisionNormal);

		// make sure the motion of the entity requires resolution
		if (dotRelativeAndNormal < 0.0f) {
			final float coefficientOfRestitution = getBounciness(); // 0 =
																	// perfectly
																	// inelastic,
																	// 1 =
																	// perfectly
	...
	protected void resolveCollision(Vector2 velocity, Vector2 impulse,
			Vector2 opposingNormal, float otherMass, Vector2 otherVelocity,
		...
		if (dotRelativeAndNormal < 0.0f) {
			final float bounciness = Math.min(
					getBounciness() + otherBounciness, 1.0f);
			final float coefficientOfRestitution = bounciness; // 0 = perfectly
																// inelastic,
																// 1 = perfectly
																// elastic
			...
			/*
			 * Vector3 entity2Adjust = (collisionNormal j); entity2Adjust[0] /=
			 * otherMass; entity2Adjust[1] /= otherMass; entity2Adjust[2] /=
			 * otherMass;
			 * 
