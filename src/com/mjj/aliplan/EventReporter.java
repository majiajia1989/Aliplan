...
public class EventReporter implements Runnable {
	...
	public void addEvent(int eventType, float x, float y, float time,
			String level, int version, long session) {
	...
	public void recordEvent(Event event) {
		...
		if (REPORT_SERVER != null) {
			try {
				serverAddress = new URL(REPORT_SERVER + "?" + "event="
						+ event.eventType + "&x=" + event.x + "&y=" + event.y
						+ "&time=" + event.time + "&level="
						+ URLEncoder.encode(event.level, "UTF-8") + "&version="
						+ event.version + "&session=" + event.session);
				...
				DebugLog.d("Report Event", event.eventType + "  " + response
