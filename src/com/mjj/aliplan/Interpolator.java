...
public class Interpolator extends AllocationGuard {
	...
	// Rather than simply interpolating acceleration and velocity for each time
	// step
	// (as in, position += (velocity * time); velocity += (acceleration *
	// time);),
	// we actually perform the work needed to calculate the integral of velocity
	// with respect to
	// time.
	//
	...
	public float interpolate(float secondsDelta) {
		...
		// there
		float directionalAcceleration = calculateAcceleration(oldVelocity,
				mAcceleration, mTarget);

		// calculate scaled acceleration (0.5 * acceleration * (time^2))
		float scaledAcceleration;
		scaledAcceleration = scaleAcceleration(directionalAcceleration,
				secondsDelta);

		// calculate the change in position
		float positionOffset = (oldVelocity * secondsDelta)
				+ scaledAcceleration;

		// change in velocity = v + aT
		float newVelocity = oldVelocity
				+ (directionalAcceleration * secondsDelta);
	...
	private boolean passedTarget(float oldVelocity, float newVelocity,
