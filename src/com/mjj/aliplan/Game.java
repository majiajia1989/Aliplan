...
public class Game extends AllocationGuard {
	...
	public void bootstrap(Context context, int viewWidth, int viewHeight,
			int gameWidth, int gameHeight, int difficulty) {
		if (!mBootstrapComplete) {
			...
			BaseObject.sSystemRegistry.customToastSystem = new CustomToastSystem(
					context);
			...
			BaseObject.sSystemRegistry
					.registerForReset(BaseObject.sSystemRegistry.channelSystem);
			...
			collision.loadCollisionTiles(context.getResources()
					.openRawResource(R.raw.collision));
			...
			hud.setFuelDrawable(
					new DrawableBitmap(longTermTextureLibrary
							.allocateTexture(R.drawable.ui_bar), 0, 0),
					new DrawableBitmap(longTermTextureLibrary
							.allocateTexture(R.drawable.ui_bar_bg), 0, 0));
			hud.setFadeTexture(longTermTextureLibrary
					.allocateTexture(R.drawable.black));
			hud.setButtonDrawables(
					new DrawableBitmap(
							longTermTextureLibrary
									.allocateTexture(R.drawable.ui_button_fly_disabled),
							0, 0),
					new DrawableBitmap(longTermTextureLibrary
							.allocateTexture(R.drawable.ui_button_fly_off), 0,
							0),
					new DrawableBitmap(longTermTextureLibrary
							.allocateTexture(R.drawable.ui_button_fly_on), 0, 0),
					new DrawableBitmap(longTermTextureLibrary
							.allocateTexture(R.drawable.ui_button_stomp_off),
							0, 0),
					new DrawableBitmap(longTermTextureLibrary
							.allocateTexture(R.drawable.ui_button_stomp_on), 0,
							0),
					new DrawableBitmap(
							longTermTextureLibrary
									.allocateTexture(R.drawable.ui_movement_slider_base),
							0, 0),
					new DrawableBitmap(
							longTermTextureLibrary
									.allocateTexture(R.drawable.ui_movement_slider_button_off),
							0, 0),
					new DrawableBitmap(
							longTermTextureLibrary
									.allocateTexture(R.drawable.ui_movement_slider_button_on),
							0, 0));
			...
			DrawableBitmap xDrawable = new DrawableBitmap(
					longTermTextureLibrary.allocateTexture(R.drawable.ui_x), 0,
					0);

			hud.setDigitDrawables(digits, xDrawable);
			hud.setCollectableDrawables(
					new DrawableBitmap(longTermTextureLibrary
							.allocateTexture(R.drawable.ui_pearl), 0, 0),
					new DrawableBitmap(longTermTextureLibrary
							.allocateTexture(R.drawable.ui_gem), 0, 0));
	...
	protected synchronized void stopLevel() {
		...
		mSurfaceView
				.flushTextures(BaseObject.sSystemRegistry.shortTermTextureLibrary);
	...
	protected synchronized void goToLevel(LevelTree.Level level) {
		...
		BaseObject.sSystemRegistry.levelSystem.loadLevel(level, params.context
				.getResources().openRawResource(level.resource), mGameRoot);
		...
		mSurfaceView
				.loadTextures(BaseObject.sSystemRegistry.shortTermTextureLibrary);
		...
		if (toast != null) {
			if (level.inThePast) {
				...
				if (mLastLevel != null && mLastLevel.inThePast) {
					toast.toast(context
							.getString(R.string.memory_playback_complete),
							Toast.LENGTH_LONG);
				}
			}
		}
