...
import com.mjj.aliplan.ConversationUtils;
import com.mjj.aliplan.ConversationUtils.Conversation;
import com.mjj.aliplan.ConversationUtils.ConversationPage;
...
public class ConversationDialogActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// LevelTree.get(mLevelRow,
		// mLevelIndex).dialogResources.character2Entry.get(index)
		if (levelRow != -1 && levelIndex != -1 && index != -1) {
			if (character == 1) {
				mConversation = LevelTree.get(levelRow, levelIndex).dialogResources.character1Conversations
						.get(index);
			} else {
				mConversation = LevelTree.get(levelRow, levelIndex).dialogResources.character2Conversations
						.get(index);
			}
	...
	private void formatPages(Conversation conversation, TextView textView) {
		...
		for (int page = conversation.pages.size() - 1; page >= 0; page--) {
			ConversationUtils.ConversationPage currentPage = conversation.pages
					.get(page);
			...
			int lastPageStart = 0;
			do {
				int fittingChars = paint.breakText(text, currentOffset,
						textLength, true, maxWidth, null);

				if (currentOffset + fittingChars < textLength) {
					fittingChars -= 2;
					// Text doesn't fit on the line. Insert a return after the
					// last space.
					int lastSpace = TextUtils.lastIndexOf(text, ' ',
							currentOffset + fittingChars - 1);
					if (lastSpace == -1) {
						// No spaces, just split at the last character.
						lastSpace = currentOffset + fittingChars - 1;
					}
					spannedText
							.replace(lastSpace, lastSpace + 1, newline, 0, 1);
					lineCount++;
				...
				if (lineCount >= maxLinesPerPage || currentOffset >= textLength) {
					...
					if (addedPages == 0) {
						...
						currentPage.text = spannedText.subSequence(
						...
						newPage.text = spannedText.subSequence(lastPageStart,
								currentOffset);
	...
	public static class TypewriterTextView extends TextView {
		...
		private ConversationDialogActivity mParentActivity; // This really
															// sucks.
		...
		public long getRemainingTime() {
			return (mText.length() - mCurrentCharacter)
					* TEXT_CHARACTER_DELAY_MS;
		}
			...
			if (delta > TEXT_CHARACTER_DELAY_MS) {
				if (mText != null) {
					if (mCurrentCharacter <= mText.length()) {
						CharSequence subtext = mText.subSequence(0,
								mCurrentCharacter);
