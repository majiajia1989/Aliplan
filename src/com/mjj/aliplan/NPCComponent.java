...
public class NPCComponent extends GameComponent {
	...
	public void update(float timeDelta, BaseObject parent) {
		...
		if (mReactToHits && mPauseTime <= 0.0f
			...
			if (mSpawnGameEventOnDeath && mGameEvent != -1) {
				if (Utils.close(parentObject.getVelocity().x, 0.0f)
					...
					if (mDeathTime < mDeathFadeDelay
							&& mDeathTime + timeDelta >= mDeathFadeDelay) {
						HudSystem hud = sSystemRegistry.hudSystem;

						if (hud != null) {
							hud.startFade(false, 1.5f);
							hud.sendGameEventOnFadeComplete(mGameEvent,
									mGameEventIndex);
							mGameEvent = -1;
						}
					}
		...
		if (mPauseTime <= 0.0f) {
			...
			if (hotSpotSystem != null) {
				...
				final int hitTileY = hotSpotSystem.getHitTileY(parentObject
						.getPosition().y + 10.0f);
				boolean accepted = true;

				if (hitTileX != mLastHitTileX || hitTileY != mLastHitTileY) {

					final int hotSpot = hotSpotSystem.getHotSpotByTile(
							hitTileX, hitTileY);

					if (hotSpot >= HotSpotSystem.HotSpotType.NPC_GO_RIGHT
							&& hotSpot <= HotSpotSystem.HotSpotType.NPC_SLOW) {
						// movement-related commands are immediate
						parentObject.setCurrentAction(ActionType.MOVE);
						accepted = executeCommand(hotSpot, parentObject,
								timeDelta);
					} else if (hotSpot == HotSpotSystem.HotSpotType.ATTACK
							&& !mPauseOnAttack) {
						// when mPauseOnAttack is false, attacks are also
						// immediate.
						accepted = executeCommand(hotSpot, parentObject,
								timeDelta);
				...
				if (mExecutingQueue) {
					if (mQueueTop != mQueueBottom) {
						accepted = executeCommand(nextCommand(), parentObject,
								timeDelta);
						if (accepted) {
							advanceQueue();
						}
	...
	private boolean executeCommand(int hotSpot, GameObject parentObject,
			float timeDelta) {
		...
		case HotSpotSystem.HotSpotType.TALK:
			if (mHitReactComponent != null) {
				if (parentObject.lastReceivedHitType != HitType.COLLECT) {
					mHitReactComponent.setSpawnGameEventOnHit(HitType.COLLECT,
							mDialogEvent, mDialogIndex);
					...
					resumeMovement(parentObject);
					mHitReactComponent.setSpawnGameEventOnHit(HitType.INVALID,
							0, 0);
		...
		case HotSpotSystem.HotSpotType.NPC_GO_TOWARDS_PLAYER:
			...
			if (manager != null) {
				...
				if (player != null) {
					direction = Utils.sign(player.getCenteredPositionX()
		...
		case HotSpotSystem.HotSpotType.NPC_SLOW:
			parentObject.getTargetVelocity().x = mSlowHorizontalImpulse
					* Utils.sign(parentObject.getTargetVelocity().x);
			break;
	...
	private void queueCommand(int hotspot) {
		...
		if (nextSlot != mQueueTop) { // only comply if there is space left in
										// the buffer
	...
	public void setSpeeds(float horizontalImpulse, float slowHorizontalImpulse,
			float upImpulse, float downImpulse, float acceleration) {
