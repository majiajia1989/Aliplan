...
public class InputSystem extends BaseObject {
	...
	public void roll(float x, float y) {
		...
		mTrackball.press(time.getGameTime(), mTrackball.getX() + x,
				mTrackball.getY() + y);
	}
	...
	public void touchDown(int index, float x, float y) {
		...
		TimeSystem time = sSystemRegistry.timeSystem;
		// Change the origin of the touch location from the top-left to the
		// bottom-left to match
		// OpenGL space.
	...
	public void setOrientation(float x, float y, float z) {
		// The order of orientation axes changes depending on the rotation of
		// the screen.
		// Some devices call landscape "ROTAION_90" (e.g. phones), while others
		// call it
		// "ROTATION_0" (e.g. tablets). So we need to adjust the axes from
		// canonical
		...
		canonicalOrientationToScreenOrientation(mScreenRotation,
				mOrientationInput, mOrientationOutput);
		...
		TimeSystem time = sSystemRegistry.timeSystem;
		mOrientationSensor.press(time.getGameTime(), horizontalMotion,
				verticalMotion);
