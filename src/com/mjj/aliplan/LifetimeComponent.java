...
public class LifetimeComponent extends GameComponent {
	...
	public void update(float timeDelta, BaseObject parent) {
		...
		if (mDieWhenInvisible) {
			...
			if (dx > context.gameWidth || dy > context.gameHeight) {
				...
				// TODO: this is a pretty dumb test. We should have a bounding
				// volume instead.
				die(parentObject);
				return;
			}
	...
	private void die(GameObject parentObject) {
		...
		if (mReleaseGhostOnDeath) {
			// TODO: This is sort of a hack. Find a better way to do this
			// without introducing a
			// dependency between these two. Generic on-death event or
			// something.
			GhostComponent ghost = parentObject
		...
		if (mSpawnOnDeathType != GameObjectFactory.GameObjectType.INVALID) {
			GameObject object = factory.spawn(mSpawnOnDeathType,
