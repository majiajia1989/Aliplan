...
public class PlayerComponent extends GameComponent {
	...
	private final static DifficultyConstants sDifficultyArray[] = {
			new BabyDifficultyConstants(), new KidsDifficultyConstants(),
	...
	protected void move(float time, float timeDelta, GameObject parentObject) {
		...
		if (pool != null && input != null) {
			...
			if (mFuel < FUEL_AMOUNT) {
				if (mTouchingGround) {
					mFuel += mDifficultyConstants.getFuelGroundRefillSpeed()
							* timeDelta;
				} else {
			...
			if (dpad.getPressed() || jumpButton.getPressed()) {
				...
				if (jumpButton.getPressed()) {
					if (jumpButton.getTriggered(time) && mTouchingGround) {
						// In this case, velocity is instant so we don't need to
						// scale
						// it by time.
				...
				if (inTheAir) {
					...
					if (Math.abs(currentSpeed) > maxHorizontalSpeed) {
						float postDragSpeed = currentSpeed
								- (AIR_DRAG_SPEED * timeDelta * Utils
										.sign(currentSpeed));
						if (Utils.sign(currentSpeed) != Utils
								.sign(postDragSpeed)) {
							postDragSpeed = 0.0f;
						} else if (Math.abs(postDragSpeed) < maxHorizontalSpeed) {
							postDragSpeed = maxHorizontalSpeed
									* Utils.sign(postDragSpeed);
						}
	...
	public void update(float timeDelta, BaseObject parent) {
		...
		if (mInventory != null && mState != State.WIN) {
			...
			if (inventory.coinCount >= mDifficultyConstants
					.getCoinsPerPowerup()) {
				...
				if (mInvincibleEndTime < gameTime) {
					...
					mInvincibleEndTime = gameTime
							+ mDifficultyConstants.getGlowDuration();
					...
					// To extend the invincible time we need to increment the
					// value above (easy)
					// and also tell the component managing the glow sprite to
					// reset its
					// timer (not easy). Next time, make a shared value system
					// for this
					// kind of case!!
		...
		if (mInvincibleEndTime > 0.0f
				&& (mInvincibleEndTime < gameTime || mState == State.DEAD)) {
			mInvincibleSwap.activate(parentObject);
			mInvincibleEndTime = 0.0f;
			if (mHitReaction != null) {
			...
			if (attackButton.getTriggered(time) && !mTouchingGround) {
				...
				if (mGhostChargeTime > GHOST_CHARGE_TIME) {
					...
					if (factory != null && manager != null) {
						...
						GameObject ghost = factory.spawnPlayerGhost(x, y,
								parentObject, ghostTime);
	...
	protected void stateStomp(float time, float timeDelta,
			GameObject parentObject) {
		if (mTimer < 0.0f) {
			// first frame
			mTimer = time;
	...
	protected void stateDead(float time, float timeDelta,
			GameObject parentObject) {
		if (mTouchingGround
				&& parentObject.getCurrentAction() != ActionType.DEATH) {
		...
		if (parentObject.getCurrentAction() == ActionType.DEATH
			...
			if (hud != null && !hud.isFading()) {
				if (elapsed > 2.0f) {
					...
					if (recorder != null) {
						recorder.setLastDeathPosition(parentObject
								.getPosition());
					}
				}
			}
	...
	protected void statePostGhostDelay(float time, float timeDelta,
			GameObject parentObject) {
		if (time > mGhostDeactivatedTime) {
			if (!mGhostActive) { // The ghost might have activated again during
									// this delay.
				CameraSystem camera = sSystemRegistry.cameraSystem;
				if (camera != null) {
	...
	public final void deactivateGhost(float delay) {
		mGhostActive = false;
		mGhostDeactivatedTime = sSystemRegistry.timeSystem.getGameTime()
				+ delay;
		gotoPostGhostDelay();
	}
	...
	public final void adjustDifficulty(GameObject parent, int levelAttemps) {
		...
		// Also make fuel refill faster in the air after we've died too many
		// times.
		...
		if (levelAttemps >= mDifficultyConstants.getDDAStage1Attempts()) {
			if (levelAttemps >= mDifficultyConstants.getDDAStage2Attempts()) {
				...
				mFuelAirRefillSpeed = mDifficultyConstants
						.getDDAStage2FuelAirRefillSpeed();
			} else {
				parent.life += mDifficultyConstants.getDDAStage1LifeBoost();
				mFuelAirRefillSpeed = mDifficultyConstants
						.getDDAStage1FuelAirRefillSpeed();
			}
		}
