...
public final class LevelTree {
		...
		try {
			...
			while (eventType != XmlPullParser.END_DOCUMENT) {
				if (eventType == XmlPullParser.START_TAG) {
					...
					if (parser.getName().equals("level")
						...
						for (int i = 0; i < parser.getAttributeCount(); i++) {
							if (parser.getAttributeName(i).equals("past")) {
								...
								final int value = parser
										.getAttributeResourceValue(i, -1);
								if (value != -1) {
						...
						currentDialog = null;
						currentLevel = new Level(levelResource, null,
								titleString, timeStamp, inThePast, restartable,
								showWaitMessage);
					...
					if (parser.getName().equals("diary")
							&& currentDialog != null) {
					...
					if (parser.getName().equals("character2")
							&& currentDialog != null) {
				...
				if (level != null && level.dialogResources != null) {
					...
					if (dialog.character1Entry != 0) {
						dialog.character1Conversations = ConversationUtils
								.loadDialog(dialog.character1Entry, context);
					}

					if (dialog.character2Entry != 0) {
						dialog.character2Conversations = ConversationUtils
								.loadDialog(dialog.character2Entry, context);
					}
				}
			}
	...
	public final static void updateCompletedState(int levelRow,
			int completedLevels) {
